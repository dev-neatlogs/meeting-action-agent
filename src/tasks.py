...
def build_tasks(transcript: str) -> list[Task]:
    ...
    publish_to_notion = Task(
        description=(
            ...
            "Call the Publish Action Items tool exactly ONCE with the full JSON array "
            "of enriched action items. The tool must publish one Notion page per item "
            "(no skipping).\n\n"
            "After the tool call, return a PUBLISH SUMMARY:\n"
